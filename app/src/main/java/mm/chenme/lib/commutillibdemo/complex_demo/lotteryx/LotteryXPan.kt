 ...
 */
class LotteryXPan @JvmOverloads constructor(
    ...
    private var mTextSize = sp(11).toFloat()
    private var mTextTopMargin = dip(77).toFloat()
    private var mImgTopMargin = dip(37).toFloat()
    private val mOutMargin = dip(32).toFloat() // 白圈的宽度

    private var mPreAngle = 0f // 每一份扇形的角度


    private val mBonusList = mutableListOf<BonusBean>()
    private val mColorList = listOf(color(R.color.color_white_fafafa), Color.WHITE)
...
//        mPaint.color = color(R.color.color_white_cc)
        canvas.drawCircle(circleXY, circleXY, circleXY-mOutMargin, mPaint)
        ...
        mBonusList.forEachIndexed { index, bonusBean ->
            ...
            canvas.drawBitmap(mImgList[index], circleXY - dip(20), mImgTopMargin, mPaint)
